...
public class ImageUploadService implements CommandProcess {
	...
	public String requestpro(HttpServletRequest request, HttpServletResponse response) throws Throwable {
		try {
			// 파일 정보
	        String sFileInfo = "";
	        String sFilename = request.getParameter("profileName");  // 파일 이름 수정
	        System.out.println(sFilename);
	        if (sFilename == null) {
	            throw new NullPointerException("sFilename is null");  // 파일 이름이 null일 경우 예외 발생
	        }
			...
			if (!isAllowed) {
				...
				sFileInfo += "&sFileURL=" + "https://kr.object.ncloudstorage.com/" + bucketName + "/" + directoryPath + uploadedFileName;
